...
(function ($) {
    $.alert_ext = {
        ...
        defaults: {
            ...
            type: 'danger',  // 提示类型
            position: ['top-right'], // 位置,第一个写位置，英文哦，后面是偏移，如果是1跟-1之间为百分比
            title: false, // 标题
            close: '',   // 需绑定关闭事件滴按钮
            speed: 'normal',   // 速度
            isOnly: true, //是否只出现一个
        ...
        // 提示框模版
        tmpl: '<div class="alert alert-dismissable ${State}" id="alert"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h4 style="white-space: nowrap;">${Title}</h4><p>${Content}</p></div>',

        // 初始化函数
        init: function (msg, options, content) {
            this.options = $.extend({}, this.defaults, options);

            this.create(msg, content);
            this.set_css();
        ...
        create: function (msg, content) {
            ...
            if (this.options.type == 'danger') {
                icon = "<p id='alertErr'><i class='fas fa-times-circle'></i> Ошибка</p>"
                this.alertDiv = this.template(this.tmpl, {
                    State: 'my-alert-' + this.options.type,
                    Title: this.options.title,
                    Content: icon + "<div class='alertbody'><div class='msg'>" + msg + "</div>" +
            ...
            if (!this.options.title) {
                ...
                $('p', this.alertDiv).css('margin-right', '75px');
                $('p', this.alertDiv).css('margin-bottom', '0px');
            }
        ...
        bind_close: function () {
            ...
            alertDiv.bind('click', function (e) {

                if (e.target.id == "alertErr") {
                    $(".content").toggle();
                    return;
                }
    ...
    $.alert = function (msg, arg, content) {
        ...
        return $.alert_ext.init(msg, arg, content);
    }
